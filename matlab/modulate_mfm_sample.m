function sim_stack = modulate_mfm_sample(sample,modulation,otf,period,phases, grab_planes, z_step, num_orientations)
...
%%
    ...
    if limit_z
       if grab_planes
           ...
           z_dims = phases*num_orientations;
...
%     fprintf('z_dims: %d\n', z_dims);
    ...
    if(grab_planes)
        mid = 257;
        del = 21;
        %slices = [249, 251, 253, 255, 257, 259, 261, 263, 265]; %70nm spacing
        %slices = [245, 248, 251, 254, 257, 260, 263, 266, 269]; %108nm spacing
        %slices = [241, 245, 249, 253, 257, 261, 265, 269, 273]; %our 144nm spacing
        %slices = [222, 229, 236, 250, 257, 264, 271, 278, 285]; %our 144nm spacing
        slices = [mid - del*4, mid - del*3, mid - del*2, mid - del, mid, mid + del, mid + del*2, mid + del*3, mid + del*4]; %our 144nm spacing
        ...
        [r, c] = size(slices);    
        fprintf('c: %f, x_dims: %f\n',c, x_dims);
        sim_stack = single(zeros(x_dims, x_dims, z_dims, c));
    else
        sim_stack = single(zeros(x_dims, x_dims, z_dims, phases));
    end
...
%       fprintf("Outer Loop %d\n", ii);
        for jj = 1:phases
            ...
            %shift_amount = round(period/7 * 3 * (jj-1)); %pay no attention to the man behind the curtain
            shift_amount = round(period/5 * (jj-1));
      
            mod_shift = gpuArray(cat(2, modulation(:,shift_amount + 1:end,:), modulation(:,1:shift_amount, :)));
            axial_step = 0; %% Top Secret
            %fprintf('angles: %d\n', angles(ii));
            mod_shift = imrotate(mod_shift ,angles(ii),'bicubic','crop');
            %mod_shift = imrotate(mod_shift ,105,'bicubic','crop');
            % this is only for non rotated or shifted image
            mod_shift(:,:,(jj - 1)*axial_step + 1:end) = mod_shift(:,:,1:end - (jj - 1)*axial_step);
            if jj > 1
                mod_shift(:,:,1:(jj - 1)*axial_step) = zeros(x_dims, x_dims,(jj - 1)*axial_step);
            end
...
%                         im2mod = single(gpuArray(zeros(x_dims, x_dims, x_dims)));
%                         im2mod(:,:,lead_im:stop_im) = sample(:,:,:); 
                        modulated = single(gpuArray((sample) .* mod_shift));
...
%                         fprintf("lead_og: %d, stop_og: %d, delta: %d\n", lead_og, stop_og, stop_og - lead_og);
                        ...
                        sim_stack(:,:,(jj) + (ii - 1)*phases, pp) = single(abs(intIm(:,:,center)));
                    end
                else
                    %im2mod = (zeros(x_dims, x_dims, x_dims));
                    %im2mod(:,:,lead_im:stop_im) = sample(:,:,lead_og:stop_og); 
                    modulated = gpuArray((sample) .* mod_shift);
                    ...
                    %break
                    sim_stack(:,:,:,jj) = single(abs(intIm(:,:,lead_og:stop_og)));
